...
impl<'a, T: AsRef<str> + IntoUrl> ParserLoop<'a, T> {
    ...
    where
        T: AsRef<str>,
    {
        let html_id_client_state = format!("{{\"AjaxTargetControl\":\"{0}\",\"Value\":\"{0}\"}}",html_id.as_ref());
        let mut date_form: HashMap<&'static str, String> = HashMap::from([
            ("RadScriptManager1", "RadToolTipManager1RTMPanel|RadToolTipManager1RTMPanel".into()),
            ("__EVENTTARGET", "RadToolTipManager1RTMPanel".into()),
            ("__EVENTARGUMENT", "undefined".into()),
            (
                "RadToolTipManager1_ClientState",
                html_id_client_state,
            ),
        ]);
        date_form.extend(base_validation);
        date_form
    ...
    pub(crate) async fn get_base_validation(
        ...
        let node_ref = kuchiki::parse_html().one(std::str::from_utf8(bytes.as_ref())?);

        let view_state_dom = node_ref
        ...
        let view_state_generator_dom = node_ref
            .select_first("#__VIEWSTATEGENERATOR")
            .unwrap();
        ...
        let event_validation_dom = node_ref
            .select_first("#__EVENTVALIDATION")
            .unwrap();
        ...
        Ok(HashMap::from([
            ...
            ("__VIEWSTATEGENERATOR", view_state_generator.to_string()),
            ("__EVENTVALIDATION", event_validation.to_string()),
        ]))
    }
}
