...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut looping = EventLoop::new(tx.clone(),&mut stream, &mut sink, client, config.get_url(), config.get_timeout(),config.get_max_concurrent()).await?;

    looping.start(tx.clone()).await;
